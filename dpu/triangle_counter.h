...
uint32_t count_triangles(__mram_ptr edge_t* sample, uint32_t edges_in_sample, triplet_t handled_triplet, uint32_t num_locations, __mram_ptr void* AFTER_SAMPLE_HEAP_POINTER, void* wram_buffer_ptr, dpu_arguments_t* DPU_INPUT_ARGUMENTS_PTR);  //to is excluded
