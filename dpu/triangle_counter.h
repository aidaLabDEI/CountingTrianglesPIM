...
bool is_triplet_handled(uint32_t c1, uint32_t c2, uint32_t c3, triplets_array_t* handled_triplets_ptr, dpu_arguments_t* DPU_INPUT_ARGUMENTS_PTR);

//from and to are used to divide the workload between tasklets
uint32_t count_triangles(__mram_ptr edge_t* sample, uint32_t from, uint32_t to, triplets_array_t* handled_triplets_ptr, uint32_t num_locations, __mram_ptr void* AFTER_SAMPLE_HEAP_POINTER, void* wram_buffer_ptr, dpu_arguments_t* DPU_INPUT_ARGUMENTS_PTR);  //to is excluded
