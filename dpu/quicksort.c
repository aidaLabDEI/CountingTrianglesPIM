...
void sort_sample(uint32_t edges_in_sample, __mram_ptr edge_t* sample_from, __mram_ptr void* sorted_sample_to,
                 edge_t* wram_buffer_ptr, uint32_t max_node_id) {
	...
	edge_t pivot_prev =
	    (edge_t){max_node_id, max_node_id}; // Best performance with starting pivot at the highest node id present
	...
	reorder(sample_from + base_tasklet, sorted_sample_to, wram_buffer_ptr, (uint64_t(*)[NR_TASKLETS])indices_loc,
	...
	while (split_task < NR_SPLITS) {
		...
		if (nr_edges_tasklets > 0) {
			sort_full(sorted_sample_to + byte_offset, sorted_sample_to + byte_offset, nr_edges_tasklets,
			          wram_buffer_ptr);
		}
