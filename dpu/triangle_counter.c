...
uint32_t count_triangles(__mram_ptr edge_t* sample, uint32_t edges_in_sample, uint32_t num_locations,
                         __mram_ptr void* FREE_SPACE_HEAP_POINTER, void* wram_buffer_ptr) {
	...
	while (sample_buffer_index < edges_to_read || global_sample_read_offset < edges_in_sample) {
		...
		node_loc_t v_info = get_location_info(num_locations, v, FREE_SPACE_HEAP_POINTER, bin_search_buffer,
...
node_loc_t get_location_info(uint32_t unique_nodes, uint32_t node_id, __mram_ptr void* FREE_SPACE_HEAP_POINTER,
	...
	while (low <= high) {
		...
		if ((uint32_t)(high - low + 1) > max_node_loc_in_buffer) {
			...
			mram_read((__mram_ptr void*)(FREE_SPACE_HEAP_POINTER + mid * sizeof(node_loc_t)), &current_node,
			...
			*node_locs_in_bin_search_buffer = (high - low + 1);
			...
			mram_read((__mram_ptr void*)(FREE_SPACE_HEAP_POINTER + low * sizeof(node_loc_t)), node_loc_buffer_ptr,
